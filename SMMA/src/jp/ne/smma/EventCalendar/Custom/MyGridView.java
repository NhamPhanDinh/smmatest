...
import jp.ne.smma.EventCalendar.Controller.TwoWayGridView;
...
public class MyGridView extends TwoWayGridView implements OnScrollListener {
	...
	public void addHeaderView(View v, Object data, boolean isSelectable) {
		...
		this.setPadding(this.getPaddingLeft(),
				topPadding + v.getMeasuredHeight(), this.getPaddingRight(),
	...
	private void setupView(View v) {
		// in my application the views are merely just inflated... because of
		// this measure and layout need to be called

		boolean isLayedOut = !((v.getRight() == 0) && (v.getLeft() == 0)
				&& (v.getTop() == 0) && (v.getBottom() == 0));

		// no need to do this more than nce per view. Sometimes people repeat
		// headers.
...
//		paint.setStrokeWidth(getMeasuredWidth()/getChildCount());
		//draw line
//		 canvas.drawLine(getChildAt(1).getX(), 0, getChildAt(1).getX(), getMeasuredHeight(), paint);
	}
	...
	public void onScroll(AbsListView view, int firstVisibleItem,
		...
		if (this.getAdapter() != null) {
			...
			if (count > 0 && firstVisibleItem == 0) {
				...
				mScrollOfsset = totalHeaderHeight - child.getTop()
						+ initialTopPadding;
	...
	private boolean removeFixedViewInfo(View v, ArrayList<FixedViewInfo> where) {
		...
		for (int i = 0; i < len; ++i) {
			...
			if (info.view == v) {
				this.setPadding(this.getPaddingLeft(),
						getPaddingTop() - v.getMeasuredHeight(),
